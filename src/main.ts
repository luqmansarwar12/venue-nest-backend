...
export class VenueNestApp{
  ...
  static port = constants.APP_PORT || 3000
  ...
  private static setupSwagger(){
    const config = new DocumentBuilder().setTitle('VenueNest API').setDescription('VenueNest API Documentation')
    .setVersion('1.0')
    .addBearerAuth(
      {
        type: 'http',
    ...
    SwaggerModule.setup('explorer', this.app, swaggerDocument)
  }
    ...
    this.app.useGlobalPipes(new ValidationPipe(
     { transform: true,
       ...
       exceptionFactory: (errors: ValidationError[]) => {
        ...
        return new HttpException(`${firstErrorMessage}`, HttpStatus.UNPROCESSABLE_ENTITY);
      },
    ...
    for (const error of errors) {
      if (error.constraints) {
        errorsException[error.property] = Object.values(
          error.constraints,
        ).shift() || 'Unknown validation error';
